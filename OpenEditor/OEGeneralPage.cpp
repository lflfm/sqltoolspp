    ...
    , m_locale(_T(""))*/
{
	...
	m_FindObjectMoveCursor	  = settings->GetFindObjectMoveCursor()    ? TRUE : FALSE;
...
void COEGeneralPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_FINDOBJECT_MOVE_CURSOR, m_FindObjectMoveCursor);
...
BOOL COEGeneralPage::OnApply()
{
    try { EXCEPTION_FRAME;
		...
		settings->SetFindObjectMoveCursor(m_FindObjectMoveCursor      ? true : false,  false /*notify*/); 
