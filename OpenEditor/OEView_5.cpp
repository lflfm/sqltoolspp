...
void COEditorView::PrintHeader (CDC* pDC, CPrintInfo* pInfo)
{
    ...
    if (pc.m_hasHeader)
    {
        ...
        pDC->TextOut(pc.m_Rulers[0](0), pc.m_Rulers[1](-2), text.c_str(), text.size());

        pc.m_header.GetHeaderPart(text, PrintHeader::epCenter, pInfo->m_nCurPage, pc.m_Rulers[1].m_Count);
        pDC->SetTextAlign(TA_CENTER);
        pDC->TextOut(pc.m_Rulers[0](pc.m_Rulers[0].m_Count/2), pc.m_Rulers[1](-2), text.c_str(), text.size());
