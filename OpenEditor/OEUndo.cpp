...
void UndoInsert::Undo (UndoContext& cxt) const
{
    ...
    cxt.m_storage.DeleteLinePart(cxt.m_position.line, cxt.m_position.column, cxt.m_position.column + m_str.length());
}

void UndoInsert::Redo (UndoContext& cxt) const
{
    cxt.m_position = m_position;
    cxt.m_storage.InsertLinePart(cxt.m_position.line, cxt.m_position.column, m_str.data(), m_str.length());
...
void UndoInsertLine::Redo (UndoContext& cxt) const
{
    cxt.m_position.column = 0;
    cxt.m_position.line = m_line;
    cxt.m_storage.InsertLine(cxt.m_position.line, m_str.data(), m_str.length());
}
...
void UndoInsertLines::Undo (UndoContext& cxt) const
{
    cxt.m_position.column = 0;
    cxt.m_position.line = m_line;
    cxt.m_storage.DeleteLines(m_line, m_lines.size());
}
...
void UndoDeleteLines::Redo (UndoContext& cxt) const
{
    cxt.m_position.column = 0;
    cxt.m_position.line = m_line;
    cxt.m_storage.DeleteLines(cxt.m_position.line, m_lines.size());
}
...
UndoReorder::UndoReorder (int top, const vector<int>& order)
: m_top(top), m_order(order)
{
}
...
void UndoReorder::Undo (UndoContext& cxt) const
{
    ...
    cxt.m_storage.Reorder(m_top, reversed);
}
...
void UndoReorder::Redo (UndoContext& cxt) const
{
    cxt.m_position.column = 0;
    cxt.m_position.line = m_top;
    cxt.m_storage.Reorder(m_top, m_order);
}
