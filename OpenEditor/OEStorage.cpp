...
void Storage::TruncateSpaces (bool force)
{
    if (force || GetSettings().GetTruncateSpaces())
    {
        ...
        for (int line(0), nlines = m_Lines.size(); line < nlines; line++)
        {
            ...
            FixedString& orgStr = m_Lines[line];
            ...
            if (newStr.size() != orgStr.size())
            {
                PUSH_IN_UNDO_STACK(new UndoDelete(line, newStr.size(), 
                    orgStr.data() + newStr.size(), orgStr.size() - newStr.size()));
                orgStr = newStr;
            }
        }
    }
...
void Storage::DeleteLine (int line)
{
	if (line < static_cast<int>(m_Lines.size()))
	{
        ...
        m_Lines.erase(line);
        PUSH_IN_UNDO_STACK(new UndoDeleteLine(line, str.data(), str.length()));
...
void Storage::Reorder (int top, const vector<int>& order)
{
	...
	if (top < nlines && top + static_cast<int>(order.size()) <= nlines)
	{
        ...
        PUSH_IN_UNDO_STACK(new UndoReorder(top, order));
