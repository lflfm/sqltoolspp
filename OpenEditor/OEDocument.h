...
public:
    ...
    static void ShowSettingsDialog (SettingsDialogCallback* = 0);
    ...
    void SetClassSetting (const char* name)         { if (name) m_settingsManager.SetClassSettingsByLang(name, m_settings); }
    void SetSaveAsToDefault ()                      { m_settings.SetFileSaveAs(m_settings.GetFileCreateAs()); }
