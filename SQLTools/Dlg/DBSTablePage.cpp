...
void CDBSTablePage::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Check(pDX, IDC_DBSP_GROUP_BY_DDL,          m_DDLSettings.m_bGroupByDDL);
    DDX_Check(pDX, IDC_DBSP_CONSTRAINTS,           m_DDLSettings.m_bConstraints);
    DDX_Check(pDX, IDC_DBSP_INDEXES,               m_DDLSettings.m_bIndexes);
    DDX_Check(pDX, IDC_DBSP_NO_STORAGE_FOR_PKUK,   m_DDLSettings.m_bNoStorageForConstraint);
    DDX_Radio(pDX, IDC_DBSP_STORAGE_DISABLE,       m_DDLSettings.m_nStorageClause);
    DDX_Check(pDX, IDC_DBSP_ALWAYS_PUT_TABLESPACE, m_DDLSettings.m_bAlwaysPutTablespace);
    DDX_Check(pDX, IDC_DBSP_TRIGGERS,              m_DDLSettings.m_bTriggers);
    DDX_Check(pDX, IDC_DBSP_TABLE,                 m_DDLSettings.m_bTableDefinition);
    DDX_Check(pDX, IDC_DBSP_COMMENTS_AFTER_COLUMN, m_DDLSettings.m_bCommentsAfterColumn);
    DDX_Text (pDX, IDC_DBSP_COMMENTS_POS,          m_DDLSettings.m_nCommentsPos);
