...
// 22.10.2006 B#XXXXXXX - (ak) compatibily issue with Oracle 7 and 8.0.X
...
// #pragma message ("   Loss of functionality - dll and database trigger support")
    ...
    void load_trigger (
        ...
        const char* owner, const char* name, bool byTables, bool useLike, size_t maxTextSize
        );

void Loader::LoadTriggers (const char* owner, const char* name, bool byTables, bool useLike)
{
#if TEST_LOADING_OVERFLOW
    load_trigger(m_connect, m_dictionary, owner, name, byTables, useLike, 32);
#else
    load_trigger(m_connect, m_dictionary, owner, name, byTables, useLike, 1024);
#endif
}
        ...
        )
    {
        ...
        subst.AddPair("<OWNER>", byTables  ? "table_owner" : "owner");
        subst.AddPair("<NAME>",  byTables  ? "table_name"  : "trigger_name");
        subst.AddPair("<ACTION_TYPE>", con.GetVersion() > OCI8::esvServer80X ? "action_type" : "");
                ...
                for (unsigned i = 0; i < sizeof(progression)/sizeof(progression[0]); i++)
                    if (maxTextSize < progression[i]) {
                        load_trigger(con, dict, owner, cur.ToString(cn_trigger_name).c_str(),
                                    false/*byTables*/, false/*useLike*/, progression[i]);
                        break;
                    }
