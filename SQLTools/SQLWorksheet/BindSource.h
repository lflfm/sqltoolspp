...
public:
    ...
    bool IsTextField (int row, int col) const; 
	int Append(const BindMapKey& name, EBindVariableTypes type, ub2 size=1);
	void Refresh();
	void Clear();
	void Delete(int row);
...
protected:
	OciConnect& GetConnect() {return(CPLSWorksheetDoc::GetApp()->GetConnect()); } 
