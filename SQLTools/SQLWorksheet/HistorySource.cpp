    ...
    m_fileTime(0)
{
    ...
    SetColCharWidth(CONNECT_COLUMN, 15);
...
void HistorySource::AddStatement (time_t startTime, const string& connectDesc, const string& sqlSttm)
{
    string sttm = sqlSttm;
    ...
    if (!sttm.empty() && settings.GetHistoryEnabled())
    {
        ...
        if (settings.GetHistoryDistinct()) 
        {
            ...
            {
                const string& str = GetCellStr(row, STATEMENT_COLUMN);
                if (str == sttm)
                {
        ...
        while (int rows = GetRowCount())
        {
            ...
            m_memoryAllocated -= GetCellStr(rows-1, STATEMENT_COLUMN).size();
            Delete(rows-1);
        }
...
void HistorySource::LoadStatement (const string& startTime, const string& connectDesc, const string& sqlSttm, bool atTop)
{
