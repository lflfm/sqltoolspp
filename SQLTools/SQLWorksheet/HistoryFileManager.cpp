...
#include "OpenEditor\OEStreams.h"
    ...
    const char* TheHistoryFileVersion   = "1002";
    const int   SQL_TEXT_OFFSET_1001    = 20;
    const int   CON_DESC_OFFSET         = 20;
    const int   SQL_TEXT_OFFSET         = 40;
    ...
    {
        return AppGetFileAttrs(filename, 0);
    }
...
HistoryFileManager& HistoryFileManager::GetInstance ()
{
    ...
    {
        string path;
        Global::GetHistoryPath(path);
        theInstance.SetFolder(path.c_str());
    }
            ...
            else if (buffer == TheHistoryFileVersion)
            {
                ...
                while (getline(in, buffer))
                {
                    if (buffer.length() > SQL_TEXT_OFFSET)
                    {
                        ...
                        // substr
                        time = buffer.substr(0, CON_DESC_OFFSET-1);
                        trim_symmetric(time);
...
bool HistoryFileManager::merge (istream& in1, istream& in2, ostream& out)
{
    ...
    while (true)
    {
        ...
        if (!buffer[0].empty() && !buffer[1].empty())
            index = (strncmp(buffer[0].c_str(), buffer[1].c_str(), CON_DESC_OFFSET) > 0) ? 0 : 1;
