...
#define DECLARE_STRICT_GRID_RUL_PROPERTY(type, name) \
    ...
    public: type Get##name () const    { return m_##name; } \
    public: void Set##name (type name) { if (m_##name != name) { On##name##Changing(); m_##name = name; On##name##Changed(); } } \
    protected: void set##name##Silently (type name) { m_##name = name; } \
    public: void Inc##name ()          { On##name##Changing(); m_##name++; On##name##Changed(); } \
    public: void Dec##name ()          { On##name##Changing(); m_##name--; On##name##Changed(); } \
    public: virtual void On##name##Changing () = 0; \
    public: virtual void On##name##Changed () = 0;
    ...
    public:
        GridRulerBase ()            { memset(this, 0, sizeof(*this)); m_SizeAsOne = -1; }
        virtual void Clear ()       { memset(&m_CellMargin, 0, (const char*)&m_Dir - (const char*)&m_CellMargin); m_SizeAsOne = -1; }
        virtual void Reset ()       { m_Current = m_Topmost = m_SavedTopmost = 0; }
    ...
    protected:
        nvector<int> m_Items;
        mutable nvector<int> m_cacheItems;
        mutable nvector<int> m_cacheFistFixItems;
        mutable nvector<int> m_cacheLastFixItems;
    public:
        ...
        void SetPixelSize         (int pos, int size);

        const nvector<int>& GetCacheItems () const                  { return m_cacheItems; }
        void SetCacheItems (const nvector<int>& cacheItems)         { m_cacheItems = cacheItems;  }
        void ClearCacheItems ()                                     { m_cacheItems.clear(); m_cacheFistFixItems.clear(); m_cacheLastFixItems.clear(); }
...
int GridRuler::GetPixelSize (int pos) const
{
    ...
    if (pos >= static_cast<int>(m_cacheItems.size()))
        m_cacheItems.resize(pos+1, -1);

    if (m_cacheItems[pos] == -1)
        m_cacheItems[pos] = GetSource()->GetSize(pos, m_Dir) * GetCharSize()  + 2 * GetCellMargin();

    return m_cacheItems[pos];
}
...
inline
void GridRuler::SetPixelSize (int pos, int size)
{
	if (pos >= static_cast<int>(m_cacheItems.size()))
		m_cacheItems.resize(pos + 1, -1);

    m_cacheItems.at(pos) = size;
}
...
struct CalcGridManager : public AGridManager
{
    ...
    virtual void AfterMove    (eDirection) = 0;
    ...
    const nvector<int>& GetCacheItems (eDirection dir) const                { return m_Rulers[dir].GetCacheItems(); }
    void SetCacheItems (eDirection dir, const nvector<int>& cacheItems)     { m_Rulers[dir].SetCacheItems(cacheItems);  }
...
struct EditGridManager : public PaintGridManager
{
    ...
    virtual void AfterMove    (eDirection);
