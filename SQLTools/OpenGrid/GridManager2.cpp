...
void PaintGridManager::erase (CDC&, const CRect&) const
{
    //dc.FillRect(rect, &CBrush(m_Dcc.m_BkColor)); //XXXXXXXX
}
...
void PaintGridManager::drawUnused (CDC& dc, const CRect& rect) const
{
    ...
    dc.FillRect(rc, &CBrush(m_Dcc.m_UnusedColor)); //XXXXXXXX
}
...
void PaintGridManager::DrawFocus (CDC& dc) const
{
    ...
    && m_Rulers[edVert].GetDataItemCount())
    {
        ...
        if (m_Options.m_ColSelect && m_Rulers[edHorz].IsCurrentVisible())
        {
            int loc[2];
        ...
        else if (m_Options.m_RowSelect && m_Rulers[edVert].IsCurrentVisible()) 
        {
            int loc[2];
...
void PaintGridManager::OnChangedRows (int startRow, int endRow)
{
    ...
    {
#pragma message("\tTODO: it can be implemented more effective!\n")

        nvector<int> items = m_Rulers[edVert].GetCacheItems();
        for (int i = startRow; i <= endRow; i++)
            items.at(i) = -1;
        m_Rulers[edVert].SetCacheItems(items);
