...
: m_Items("GridRuler::m_Items"),
  m_cacheItems("GridRuler::m_cacheItems"),
...
void GridRuler::OnTopmostChanged ()
{
...
void GridRuler::OnChangedQuantity (int pos, int quantity)
{
    if (quantity > 0) // inserting
    {
        ...
        // shift cached items
        if (pos <= (int)m_cacheItems.size())
            m_cacheItems.insert(m_cacheItems.begin() + pos, quantity, -1);
    ...
    else if (quantity < 0) // deleting
    {
        ...
        if (pos <= (int)m_cacheItems.size())
        {
            m_cacheItems.erase(m_cacheItems.begin() + pos, 
                m_cacheItems.begin() + std::min<unsigned int>(pos - quantity, m_cacheItems.size()));
        }
