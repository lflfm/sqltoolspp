...
class StringTable
{
    nvector<StringRow> m_table;
    nvector<int>       m_charWidths;
public:
    DECLARE_STRTAB_PROPERTY(int, ColumnCount);

    StringTable ()                                              : m_table("StringTable::m_table"), m_charWidths("StringTable::m_charWidths"), m_ColumnCount(0) {}
    StringTable (int ncols, int nrows = -1);

    void Clear ()                                               { m_table.clear(); m_charWidths.clear(); }
    void Format (int ncols, int nrows = -1);

    int  GetCharWidth   (int col) const                         { return m_charWidths.at(col); }
    void SetCharWidth   (int col, int width)                    { m_charWidths.at(col) = width; }
    ...
    void GetMaxColumnLength (nvector<size_t>& result) const;
    void GetMaxRowLength (nvector<size_t>& result) const;
