...
void CExtractSchemaOptionPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_ESP_LOWER_NAMES,           m_DDLSettings.m_bLowerNames);
	DDX_Check(pDX, IDC_ESP_SCHEMA_NAME,           m_DDLSettings.m_bShemaName);

	DDX_Check(pDX, IDC_ESP_SQLPLUS_COMPATIBILITY, m_DDLSettings.m_bSQLPlusCompatibility);
	DDX_Check(pDX, IDC_ESP_OPTIMAL_VIEW_ORDER,    m_DDLSettings.m_bOptimalViewOrder);

    DDX_Check(pDX, IDC_ESP_GROUP_BY_DDL,          m_DDLSettings.m_bGroupByDDL);
    DDX_Check(pDX, IDC_ESP_NO_STORAGE_FOR_PKUK,   m_DDLSettings.m_bNoStorageForConstraint);
    DDX_Radio(pDX, IDC_ESP_STORAGE_DISABLE,       m_DDLSettings.m_nStorageClause);
    DDX_Check(pDX, IDC_ESP_ALWAYS_PUT_TABLESPACE, m_DDLSettings.m_bAlwaysPutTablespace);
    DDX_Check(pDX, IDC_ESP_COMMENTS_AFTER_COLUMN, m_DDLSettings.m_bCommentsAfterColumn);
    DDX_Text (pDX, IDC_ESP_COMMENTS_POS,          m_DDLSettings.m_nCommentsPos);
