...
void AutoCursor::Define ()
{
    ...
    for (int col = 0; col < cols; col++) 
    {
        ...
        rowSize += ((size && size < m_StringLimit) ? size : m_StringLimit);
		...
		case SQLT_CLOB:
            if (m_SkipLobs)
                ...
                fld = counted_ptr<Variable>(new CLobVar(GetConnect(), m_StringLimit));
			break;
		case SQLT_BLOB:
            if (m_SkipLobs)
                fld = counted_ptr<Variable>(new DummyVar(true));
        ...
        case SQLT_DATE:
            ...
            //    fld = counted_ptr<Variable>(new TimestampVar(GetConnect(), TimestampVar::TIMESTAMP, m_DateFormat));
		    //break;
        case SQLT_TIME:
            if (Connect::IsTimestampSupported())
